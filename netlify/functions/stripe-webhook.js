...
const PROCESSING_STALE_MS = 3 * 60 * 1000; // treat a stuck "processing" record as abandoned after this

// Fired by Stripe's checkout.session.completed webhook. Runs the same
// server-side report generation as analyze.js and emails the result,
// independent of whatever happens in the customer's browser afterward —
// this is what guarantees a paid customer never loses their report to a
// refresh/closed tab.
exports.handler = async (event) => {
  ...
  if (!email || !project.title) {
    console.error('stripe-webhook: session missing email or project metadata', session.id);
